package com.example.root.sensores;
...
public class MainActivity extends AppCompatActivity implements SensorEventListener{
    ...
    private TextView sAcel,sGyro,sProx,sMag,sLuz,sBar;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sAcel=(TextView)findViewById(R.id.sAcel);
        ...
        if(sm.getSensorList(Sensor.TYPE_ACCELEROMETER).size()==0){
            sAcel.setText("Dispositivo no cuenta con acelerometro");
        }else{
            // activando Sensor
        ...
        synchronized (this){
            switch (event.sensor.getType()){
                case Sensor.TYPE_ACCELEROMETER:
                    ...
                    float acce_x= event.values[SensorManager.DATA_X];
                    float acce_y= event.values[SensorManager.DATA_Y];
                    float acce_z= event.values[SensorManager.DATA_Z];
                    // guardar dato en Textview
                    sAcel.setText(
                            "Acelerometro:"+
                            "\nX:"+String.valueOf(acce_x)+
                            "\nY:"+String.valueOf(acce_y)+
                            "\nz:"+String.valueOf(acce_z)
                    );
                break;
